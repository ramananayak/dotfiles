...
## Running Sage in a terminal split

```
:vs | term    " sage in right pane  (splitright = true)
:sp | term    " sage in bottom pane (splitbelow = true)
```

- `i` / `a` — enter terminal mode (type commands)
- `<Esc>` — exit terminal mode → normal mode (yank output, scroll)
- `<C-h/j/k/l>` — jump between splits

Each terminal running `sage` is an independent process with its own session context.
Past sessions are saved (`history.enabled: true`) and can be reloaded.
